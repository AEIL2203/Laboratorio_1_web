...
export class PersonEditComponent {
  ...
  onSubmit() {
    ...
    swal({
      ...
      text: isEdit
        ? "¿Deseas actualizar los datos de la persona?"
        : "¿Deseas guardar la nueva persona?",
      icon: "info",
      buttons: ["Cancelar", "Aceptar"],
      dangerMode: false,
    }).then((willConfirm) => {
      if (willConfirm) {
        if (isEdit && this.data?.id) {
          this.personService
            .update(this.data.id, val)
            .subscribe({
              next: () => {
                swal("Actualizado", "La persona fue actualizada correctamente.", "success");
          ...
          this.personService
            ...
            .subscribe({
              next: () => {
                swal("Guardado", "La persona fue creada correctamente.", "success");
                this.dialogRef.close(true);
              },
              error: () => {
                swal("Error", "No se pudo guardar la persona.", "error");
              }
            });
        }
      }
